...
PROJECT_NAME=intranet-trepr
STACK_NAME=intranet-tre-pr-jus-br
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://intranet-trepr.localhost"
...
acceptance-frontend-image-build: ## Build Acceptance frontend server image
	...
	@docker build frontend -t ericof/intranet-trepr-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build: ## Build Acceptance backend server image
	@echo "Build acceptance backend"
	@docker build backend -t ericof/intranet-trepr-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start: ## Start Acceptance frontend container
	...
	@docker run --rm -p 3000:3000 --name intranet-trepr-frontend-acceptance --link intranet-trepr-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d ericof/intranet-trepr-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start: ## Start Acceptance backend container
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name intranet-trepr-backend-acceptance -d ericof/intranet-trepr-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop intranet-trepr-frontend-acceptance
	@docker stop intranet-trepr-backend-acceptance
